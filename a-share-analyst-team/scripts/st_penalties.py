...
def _is_company_victim(title: str, reason: str, content: str) -> bool:
    """检测公司是否为受害方（原告），非被处罚方。"""
    combined = f"{title} {reason} {content}"
    norm = _norm_text(combined)
    if "收到" in _norm_text(title) and "刑事判决书" in _norm_text(title):
        return True
    if "侵犯商业秘密" in norm and "被告人" in combined:
        return True
    return False
...
def _classify_target_relevance(
    ...
    # 受害者检测（不依赖 target_aliases）
    if _is_company_victim(title, reason, content):
        return "company_as_victim", False, "company_is_plaintiff_not_defendant"
