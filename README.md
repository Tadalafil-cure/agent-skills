...
| Skill | Description |
|-------|-------------|
| [XuXiaoming-StockTradingStrategy](./XuXiaoming-StockTradingStrategy/) | 徐小明技术分析策略 **v4.6.12** — 蒸馏自「投资明见」2019-2026 2124篇文章。**两种模式**：盘后全流程（两路并行+合成+博客A20后置→九节报告）和盘中实时（Step-1四状态时间路由→七段简报）。**裁决层共振驱动**：主板五指数投票，科技双创共振升降档。**震荡来路判断**（v4.6.12 新增·基本原则）：来路决定规则——上行看出口方向，下行看BS累计次数。**钝化分析层**：日线+分钟线极值法20d，bd/td双列拆分。**8组规则**原文支撑，21项反模式（A1-A27），独立数据中间层。 |
