...
| Skill | Description |
|-------|-------------|
| [XuXiaoming-StockTradingStrategy](./XuXiaoming-StockTradingStrategy/) | 徐小明技术分析策略 v4.5.2 — 四层架构（能力层/表达层/规则层/工具接入层），蒸馏自「投资明见」2019-2026 2124篇文章。**8组规则**全量原文支撑（新增R8个股与大盘分化），默认7指标实现（三合一/CHOP/MACD结构/九转/分钟线/五指数共振/**NH/NL广度双公式**），裁决出口仅5种。双Agent并行分析架构（主板/科技子Agent独立推演→父Agent跨市场合成），模板v1.5新增「个股与大盘分化」段落。独立数据中间层（akshare→build.py一键构建 + breadth.py广度引擎）。封板测试393篇一致率64.1%。 |
