...
| Skill | Description |
|-------|-------------|
| [XuXiaoming-StockTradingStrategy](./XuXiaoming-StockTradingStrategy/) | 徐小明技术分析策略 **v4.6.1** — 四层架构（能力层/表达层/规则层/工具接入层），蒸馏自「投资明见」2019-2026 2124篇文章。**8组规则**全量原文支撑（R1趋势/R2 CHOP/R3结构/R4序列/R5共振/R6分钟线/R7双市场/R8个股分化），震荡全流程框架闭环（上行出口76%/下行BS≥2→100%），铁律推论固化（市况×结构对照表）。**9项默认工具**（三合一/CHOP/MACD结构/九转/分钟线/五指数共振/NH-NL广度/博客对照/数据加载器），裁决出口5种。**20+反模式**（A1-A20含市况论证分列、博客三层隔离、工具存在即责任）。双Agent并行+输出模板v1.6九段结构。独立数据中间层（build.py一键构建 + breadth.py广度引擎 + blog_monitor.py博客对照）。封板测试393篇一致率64.1%。 |
