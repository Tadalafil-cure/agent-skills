...
description: 生成横向翻页网页 PPT（单 HTML 文件），含 WebGL 背景、章节幕封、数据大字报、图片网格等模板。提供两种风格：① "电子杂志 × 电子墨水"（衬线 + 流体背景 + 暖色） ② "瑞士国际主义"（无衬线 + 网格点阵 + IKB/柠檬黄/柠檬绿/安全橙高亮）。支持导出为 PDF + PPTX（图片型）。当用户需要制作分享 / 演讲 / 发布会风格的网页 PPT，或提到"杂志风 PPT"、"瑞士风 PPT"、"Swiss Style"、"horizontal swipe deck"、"HTML 转 PPT"、"导出 PDF"时使用。
---
...
- **在 Hermes Agent 中**：通过 `clarify` 工具做逐项澄清，优先把风格、受众、素材、截图需求这些会影响版式的输入问清楚。一次最多问 2-3 个最关键问题。
- **在其他 runtime 中**：用普通对话直接询问用户，不要假设 `clarify` 等 Hermes 专属工具可用。如果信息缺口不影响开工，先做合理假设并在回复里说明。

#### 7 问澄清清单

| # | 问题 | 为什么要问 |
|---|------|-----------|
| 1 | **风格 A 还是 B?**(电子杂志风 / 瑞士国际主义风) | **默认 A · 电子杂志风**。数据密集/纯技术汇报时切换到 B。不确定时不问，直接用 A。 |
...
mxai skill 已安装（VIP 2143 积分），提供 8+ 图片模型（seedream-5.0-pro / nano-2.0 / gpt-image-2 等）+ MJ 绘画。生成配图时**直接调用 mxai REST API**，无需 GPT-M 2.0。

**调用方式**：

```bash
curl -s -X POST https://mcp.mxai.cn/mcp/api/generate/image \
  -H "Authorization: Bearer $MX_AI_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"prompt":"...", "model":"gpt-image-2", "aspect_ratio":"16:9", "quality":"high", "resolution":"2K"}'
# → 获取 serial_no → 轮询 GET /mcp/api/task/{serial_no} → status=2 时取 image_urls
```

**参数映射**（guizang 比例 → mxai aspect_ratio）：

| guizang 比例 | mxai aspect_ratio |
|---|---|
| 21:9 | 21:9（不原生支持时用 16:9 裁切） |
| 16:9 | 16:9 |
| 16:10 | 4:3（最近似） |
| 4:3 | 4:3 |
| 3:4 / 3:2 / 1:1 | 3:4 / 3:2 / 1:1 |

**默认模型**：`gpt-image-2`（GPT Image 2，2-32 积分/张，按 resolution × count 计费）。品质要求不高时可用 `seedream-5.0`（4 积分/张）或 `seedream-5.0-pro`（6-20 积分/张）。

**GPT Image 2 专属参数**：

| 参数 | 可选值 | 默认 | 说明 |
|------|------|------|------|
| `quality` | `auto` / `low` / `medium` / `high` | `auto` | 画质等级 |
| `count` | 1–4 | 1 | 生成张数 |
| `resolution` | `1K` / `2K` / `4K` | `2K` | 输出分辨率 |

**MJ 绘画**：需 MidJourney 风格时调 `POST /mcp/api/mj/draw`，首先生成四宫格，用户选图后 `mj_action` 放大。

**提示词规则**：
- 风格 A 提示词前缀：`电子杂志风格，Monocle杂志质感，衬线排版，`
- 风格 B 提示词前缀：`瑞士国际主义风格，Swiss Style，Helvetica气质，网格系统，单一锚点色，`
- 中文 deck 提示词用中文，英文 deck 用英文
- 信息图提示词末尾加 `，简洁信息图，数据可视化，无文字标签`
- 截图再设计时提示词描述「需保留的信息内容」而非「截图本身」

**生成流程**：
1. 先读 `references/image-prompts.md` 确定图片类型和比例槽位
2. 按上述参数映射拼 mxai API 请求
3. 提交后轮询（间隔 2min）直到 status=2
4. 下载 image_url 到 `images/` 下，命名遵守 `{页号}-{语义}.png`
5. HTML 中引用 `images/xxx.png`

**注意**：生成图片只保留图像本体，不要在图片中嵌入标题、页眉、页脚、页码。
...
### Step 7 · 导出 PDF / PPTX（可选）

HTML deck 完成后，**主动问用户**是否要导出为线下可用的 PDF / PPTX：

> Deck 生成好了 ✅ 需要导出为 PDF 或 PPTX 吗？可以用于线下演讲、上传平台、印刷。默认超清 5760×3240。

用户确认后执行：

```bash
node <SKILL_ROOT>/scripts/build.mjs <deck-path> --out <output-dir>
```

| 参数 | 默认 | 说明 |
|---|---|---|
| `<deck-path>` | 必填 | 本地目录（含 index.html）或 URL |
| `--out <dir>` | `./out` | 输出目录 |
| `--scale <n>` | `3` | deviceScaleFactor。3→5760×3240 超清；文件嫌大降 2 |
| `--width <px>` | `1920` | CSS 视口宽 |
| `--height <px>` | `1080` | CSS 视口高 |

**产出**（`<out>/` 下）：
- `deck.pdf` — 多页 PDF，16:9 满版，可直接投影
- `deck.pptx` — 图片型 PPTX（每页一张满版 PNG，视觉 100% 还原）
- `frames/slide-NN.png` — 每页原始 PNG

**原理**：Playwright Chromium 逐页截图 → pdf-lib 拼 PDF + pptxgenjs 拼 PPTX。依赖已在 scripts/ 下就绪。

**故障**：内容缺失 → `--wait 4000`；文件太大 → `--scale 2`；首次跑需 Chromium（已就绪，跳过）。
...
│   └── motion.min.js          ← Motion One 动效引擎（本地 + CDN 双保险）
├── scripts/
│   ├── build.mjs              ← HTML → PDF + PPTX 转换（Playwright + pdf-lib + pptxgenjs）
│   ├── setup.sh               ← 装依赖（幂等，Chromium ~150MB）
│   ├── package.json           ← npm 依赖清单
│   └── validate-swiss-deck.mjs ← 瑞士风版式校验
└── references/
...
2. 风格默认 A · 电子杂志风（靛蓝瓷主题）。数据密集/纯技术汇报时切换到 B：
   - 风格 A:读 `themes.md` 选主题色（默认靛蓝瓷）
   - 风格 B:读 `themes-swiss.md` 选主题色
...
6. 如需配图但环境中无图片生成能力，跳过生成，使用 `assets/screenshot-backgrounds/` 的内置背景资产做程序化适配；如果是用户原始截图，先读 `screenshot-framing.md`
